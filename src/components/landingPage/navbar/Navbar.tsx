...
const Navbar = () => {
  ...
  return (
    <>
      ...
      <nav className="flex justify-between items-center px-8 shadow-lg border border-gray-100 py-1">

        <span><img src={logo} className="h-10"/></span>

        <span className="flex">
          <ul className="flex space-x-8 text-center invisible lg:visible">
            {links.map((link, index) =>
              <NavLink key={index} to={link.url} > <li className={` mt-2 lg:mt-0 border-b-4 border-white  transition-colors duration-300 ease-in-out  hover:text-blue-500 hover:border-blue-500 rounded border-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:text-gradient-to-br ${link.className}`}>{link.text}</li> </NavLink>
            )}
          </ul>
      ...
      <ul className={`bg-white shadow-md flex flex-col absolute  w-full lg:-top-96 lg:hidden text-center    ${isMenuOpen ? "top-12" : " -top-96"} transistion-all duration-500 ease-in-out `}>
        {links.map((link, index) =>
          <NavLink key={index} to={link.url} > <li className={`mx-5 px-5 p-2 mt-2 lg:mt-0 border-b-4  border-white  transition-colors duration-300 ease-in-out  hover:text-blue-500 hover:border-blue-500 rounded border-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:text-gradient-to-br ${link.className}`}>{link.text}</li> </NavLink>
        )}
      </ul>
